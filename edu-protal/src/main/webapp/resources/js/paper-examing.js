...
// 		
	...
	bindSubmit : function bindSubmit() {
		$("#question-submit button").click(function() {
			layer.confirm("确认交卷吗？",function(index){
				layer.close(index);
				examing.finishExam();
			});
		...
		request.done(function(message, tst, jqXHR) {
			...
			if (message.result == "success") {
				...
				util.success("交卷成功！", function() {
					window.location.href = document.getElementsByTagName('base')[0].href + 'student/exam-report.html?kpId=' + $("#kpId").val();
				});
			} else {
