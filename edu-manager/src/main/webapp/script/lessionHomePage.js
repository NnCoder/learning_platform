$(document).ready(function() {
 ...
 $('#more-learning-process').on('click', function(){
    layer.open({
      ...
      content: '\<\div style="padding:20px;">'+learning_process+'\<\/div>'
    });
  });
 $('#more-learning-intro').on('click', function(){
    layer.open({
      ...
      area: ['800px', '640px'],
      shadeClose: true, //点击遮罩关闭
      content: '\<\div style="padding:20px;">'+learning_intro+'\<\/div>'
    });
  });
	//显示登录的个人账号

  // 课程资源的tab
  var insertHtml = '<div class="my-tab-item"></div>';
  var tab_titles = $('.my-tab-title').find('li');
  var tab_content = $('.my-tab-content');
  var tab_item = $('#my-tab-content').find('div');
